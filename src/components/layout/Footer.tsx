        ...
        <div className="grid gap-8 sm:gap-10 md:gap-12 grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
          ...
          <nav aria-label="Services navigation">
            ...
            <ul className="space-y-2 sm:space-y-3">
              {services.map((service) => (
                <li key={service.href}>
                  <Link to={service.href} className="text-sm text-primary-foreground/70 hover:text-primary-foreground transition-colors inline-block py-1 touch-manipulation">
                    {service.title}
                  </Link>
                </li>
              ))}
          ...
          <nav aria-label="Quick links navigation">
            ...
            <ul className="space-y-2 sm:space-y-3">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  <Link to={link.href} className="text-sm text-primary-foreground/70 hover:text-primary-foreground transition-colors inline-block py-1 touch-manipulation">
                    {link.title}
                  </Link>
                </li>
              ))}
            ...
            <ul className="space-y-3 sm:space-y-4">
              <li className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <a href={`tel:${settings.business_phone.replace(/\s+/g, '')}`} className="text-sm text-primary-foreground/70 hover:text-primary-foreground transition-colors touch-manipulation">
              ...
              <li className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <a href={`mailto:${settings.business_email}`} className="text-sm text-primary-foreground/70 hover:text-primary-foreground transition-colors touch-manipulation">
          ...
          <div className="flex flex-col gap-4 sm:gap-6 md:flex-row md:items-center md:justify-between">
            ...
            <nav aria-label="Social media links" className="flex items-center justify-center gap-3 sm:gap-4 order-first md:order-none">
              {socialLinks.map((social) => (
                <a key={social.label} href={social.href} aria-label={`Follow us on ${social.label}`}
            ...
            <nav aria-label="Legal links" className="flex items-center justify-center gap-4 sm:gap-6">
              <Link to="/privacy" className="text-xs sm:text-sm text-primary-foreground/60 hover:text-primary-foreground transition-colors touch-manipulation py-1">Privacy Policy</Link>
              <Link to="/terms" className="text-xs sm:text-sm text-primary-foreground/60 hover:text-primary-foreground transition-colors touch-manipulation py-1">Terms of Service</Link>
              <Link to="/admin/login" className="text-xs sm:text-sm text-primary-foreground/60 hover:text-primary-foreground transition-colors touch-manipulation py-1">Admin</Link>
            </nav>
          </div>
        </div>
      </div>
